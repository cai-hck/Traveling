                    ...
                    <div class="datatable-wrapper">
                        <table class="datatable-badges display cell-border" style="text-align:center">
                            ...
                            <tr>
                                ...
                                <th>Child Money</th>
                                <th>Infant Money</th>
                                <th>Adult Business Money</th>
                                <th>Chlld Business Money</th>
                                <th>Room Business Money</th>
                                <th>Total Money</th>
                                <th>Created Time</th>
                            </tr>
                            </thead>
                            <tbody>
                                @foreach ($appointments as $appointment)
                                    <tr>
                                        ...
                                        <td>{{$appointment->adult}}<?php if($appointment->single == 'on') { echo'(Business Room)';} ?></td>
                                        <td>{{$appointment->child}}<?php if($appointment->childbed == 1) { echo'(Business Room)';} ?></td>
                                        <td>{{$appointment->infant}}</td>
                                        <td>{{$appointment->room}}</td>
                                        <td>{{$appointment->price_adult}}</td>
                                        <td>{{$appointment->price_child}}</td>
                                        <td>{{$appointment->price_infant}}</td>
                                        <td>{{$appointment->price_adult_b}}</td>
                                        <td>{{$appointment->price_child_b}}</td>
                                        <td>{{$appointment->price_infant_b}}</td>
                                        <?php 
                                        $money1 = $appointment->infant *  $appointment->price_infant;
                                        $money1 += $appointment->room *  $appointment->price_infant_b;  
                                        if($appointment->single == 'on') 
                                            $money1 += $appointment->adult *  $appointment->price_adult_b; 
                                        else
                                            $money1 += $appointment->adult *  $appointment->price_adult; 
                                        if($appointment->childbed == 1) 
                                            $money1 += $appointment->child *  $appointment->price_child_b; 
                                        else
